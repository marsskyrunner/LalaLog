...
/**
 * {@link ContentProvider} for LalaLog app.
 */
public class SchoolProvider extends ContentProvider {
     ...
     */
    public final String LOG_TAG = SchoolProvider.class.getSimpleName();
     ...
     */
    private static final int SUBJECTS = 102;

    /**
     * URI matcher code for the content URI for a single record in the SUBJECTS table
     */
    private static final int SUBJECT_ID = 103;
    ...
    static {
        ...
        // The content URI of the form "content://com.mars_skyrunner.lalalog.records/records" will map to the
        ...
        // For example, "content://com.mars_skyrunner.lalalog.records/records/3" matches, but
        // "content://com.mars_skyrunner.lalalog.records/records" (without a number at the end) doesn't match.
        ...
        // The content URI of the form "content://com.mars_skyrunner.lalalog.subjects/subjects" will map to the
        ...
        // The content URI of the form "content://com.mars_skyrunner.lalalog.SUBJECTS/SUBJECTS/#" will map to the
        // integer code {@link #SUBJECT_ID}. This URI is used to provide access to ONE single row
        // of the SUBJECTS table.
        //
        // In this case, the "#" wildcard is used where "#" can be substituted for an integer.
        // For example, "content://com.mars_skyrunner.lalalog.SUBJECTS/SUBJECTS/3" matches, but
        // "content://com.mars_skyrunner.lalalog.SUBJECTS/SUBJECTS" (without a number at the end) doesn't match.
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            case RECORDS:
                ...
                Log.w(LOG_TAG, "sUriMatcher:  RECORDS");
            ...
            case RECORD_ID:
                ...
                // For an example URI such as "content://com.mars_skyrunner.lalalog.records/records/3",
    ...
    private Uri insertRecord(Uri uri, ContentValues values) {
        ...
        String date = values.getAsString(RecordEntry.COLUMN_RECORD_DATE);
        if (date == null) {
            throw new IllegalArgumentException("Record requires a date");
        }

        // Check that the time is not null
        String time = values.getAsString(RecordEntry.COLUMN_RECORD_TIME);
        if (time == null) {
        ...
        String text = values.getAsString(RecordEntry.COLUMN_RECORD_TEXT);
        if (text == null) {
        ...
        String subjectID = values.getAsString(RecordEntry.COLUMN_SUBJECT_ID);
        if (subjectID == null || !RecordEntry.isValidSubjectID(subjectID)) {
            throw new IllegalArgumentException("Record requires valid subject id");
        }

        // Check that the subjectID is not null
        String subjectGroupID = values.getAsString(RecordEntry.COLUMN_SUBJECT_GROUP_ID);
...
//        }
    ...
    private int updateRecord(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // If the {@link RecordEntry#COLUMN_RECORD_DATE} key is present,
        // check that the date value is not null.
        if (values.containsKey(RecordEntry.COLUMN_RECORD_DATE)) {
            String date = values.getAsString(RecordEntry.COLUMN_RECORD_DATE);
            if (date == null) {
                throw new IllegalArgumentException("Record requires a date");
            }
        ...
        // If the {@link RecordEntry#COLUMN_RECORD_TIME} key is present,
        // check that the time value is not null.
        if (values.containsKey(RecordEntry.COLUMN_RECORD_TIME)) {
            String time = values.getAsString(RecordEntry.COLUMN_RECORD_TIME);
            if (time == null) {
                throw new IllegalArgumentException("Record requires a time");
            }
        ...
        // If the {@link RecordEntry#COLUMN_RECORD_TEXT} key is present,
        // check that the time text is not null.
        if (values.containsKey(RecordEntry.COLUMN_RECORD_TEXT)) {
            String text = values.getAsString(RecordEntry.COLUMN_RECORD_TEXT);
            if (text == null) {
                throw new IllegalArgumentException("Record requires a text");
            }
        ...
        // If the {@link RecordEntry#COLUMN_SUBJECT_ID} key is present,
        // check that the subject id is not null.
        if (values.containsKey(RecordEntry.COLUMN_SUBJECT_ID)) {
            String subjectID = values.getAsString(RecordEntry.COLUMN_SUBJECT_ID);
            if (subjectID == null || !RecordEntry.isValidSubjectID(subjectID)) {
                throw new IllegalArgumentException("Record requires valid subject id");
            }
        ...
        int rowsUpdated = database.update(RecordEntry.TABLE_NAME, values, selection, selectionArgs);
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            ...
            case RECORD_ID:
                ...
                Log.w(LOG_TAG,"delete RECORD_ID");
                selection = RecordEntry._ID + "=?";
                selectionArgs = new String[]{String.valueOf(ContentUris.parseId(uri))};
                rowsDeleted = database.delete(RecordEntry.TABLE_NAME, selection, selectionArgs);

                Log.w(LOG_TAG,"rowsDeleted: " + rowsDeleted);
                break;

            case SUBJECTS:
                // Delete all rows that match the selection and selection args

                Log.w(LOG_TAG,"delete SUBJECTS");
            ...
            case SUBJECT_ID:
                ...
                Log.w(LOG_TAG,"delete SUBJECT_ID");
                selection = SubjectContract.SubjectEntry._ID + "=?";
                selectionArgs = new String[]{String.valueOf(ContentUris.parseId(uri))};
                rowsDeleted = database.delete(SubjectContract.SubjectEntry.TABLE_NAME, selection, selectionArgs);
                break;
